...
class AverageCorrelationTest(unittest.TestCase):
    ...
    def testMicroAveraging(self):
        g, m = [1, None, 3, 4, 5, 6], [2, 1, 8, 3, 5, 6]
        cf = stats.AverageCorrelation(pearson, num_sys=2, average_by="sys", macro=False)
        p1 = pearson([1, 3], [2, 8])
        p2 = pearson([4, 5, 6], [3, 5, 6])
        c12 = cf(g, m)
...
class CorrelationFunctionsTest(unittest.TestCase):
    ...
    def testKendallVariants_b_and_c(self):
        ...
        for variant in ["b", "c"]:
            ...
            tau = stats.KendallVariants(None, metric, preproc=prep, variant=variant)[0]
            self.assertEqual(ref, tau)

            # Metric-factored version matches scipy
            pd = stats.PairwiseDiffs(metric)
            m = metric if variant == "c" else None
            tau = stats.KendallVariants(gold, m, variant=variant, metric_preproc=pd)[0]
            self.assertEqual(ref, tau)

            # Dual-factored version matches scipy
            m = metric if variant == "c" else None
            tau = stats.KendallVariants(
                None, m, variant=variant, preproc=prep, metric_preproc=pd
            )[0]
            self.assertEqual(ref, tau)
    ...
    def testKendallVariantsWithPositiveEpsilon(self):
        ...
        self.assertEqual(actual_tau, expected_tau)
        #
        actual_tau = stats.KendallVariants(x, None, metric_preproc=pd, variant="23")[0]
        self.assertEqual(actual_tau, expected_tau)
        #
        actual_tau = stats.KendallVariants(
            None, None, preproc=prep, metric_preproc=pd, variant="23"
        )[0]
        self.assertEqual(actual_tau, expected_tau)
    ...
    def testKendallWithTiesOpt(self):
        ...
        corr, eps, _ = stats.KendallWithTiesOpt(
            gold, metric, variant="acc23", num_sys=1, average_by="none", sample_rate=1.0
        )
        ref = stats.KendallVariants(gold, metric, variant="acc23", epsilon=eps)[0]
        self.assertEqual(corr, ref)

        corr, eps, _ = stats.KendallWithTiesOpt(
            gold, metric, variant="acc23", num_sys=2, average_by="sys", sample_rate=1.0
        )
        ...
        corr, eps, _ = stats.KendallWithTiesOpt(
            gold, metric, variant="acc23", num_sys=2, average_by="item", sample_rate=1.0
        )
...
class PermutationSigDiffTest(unittest.TestCase):
    ...
    def testPearsonNoAvgNoEarlyStop(self):
        delta = (
            pearson(self.metric2, self.gold)[0] - pearson(self.metric1, self.gold)[0]
        )
    ...
    def testPearsonSysNoEarlyStop(self):
        ...
        p, d, k, _ = stats.PermutationSigDiff(self.corr1, self.corr2, pearson, "sys")
        self.assertGreater(p, 0)
        self.assertAlmostEqual(d, delta)
        self.assertEqual(k, 1000)
    ...
    def testPearsonItemNoEarlyStop(self):
        ...
        delta = cf(self.gold, self.metric2)[0] - cf(self.gold, self.metric1)[0]
        p, d, k, _ = stats.PermutationSigDiff(self.corr1, self.corr2, pearson, "item")
        self.assertGreater(p, 0)
        self.assertAlmostEqual(d, delta)
        self.assertEqual(k, 1000)
    ...
    def testTauOptimization(self):
        tau1 = self.corr1.KendallWithTiesOpt(
            average_by="item", variant="acc23", sample_rate=1.0
        )[0]
