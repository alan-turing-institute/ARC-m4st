...
import dacite
from mt_metrics_eval import data, ratings
...
def _RenameRaters(ratings_list: list[Rating], anonymize: bool) -> dict[Any, str]:
    ...
    if anonymize and not all(r.startswith("rater") for r in rater_ids if r):
        ...
        return {rater: (rater if rater is not None else "rater") for rater in rater_ids}
...
def _CheckRating(
    ...
    if rating.document_id is not None:
        if rating.document_id not in evs.doc_names:
            raise ValueError(f"Unknown doc in rating {rating_id}: {rating.document_id}")
