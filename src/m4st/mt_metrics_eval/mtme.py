...
from mt_metrics_eval import data, meta_info, stats, tasks

flags.DEFINE_bool(
    "download",
    False,
    "Download local copy of the database and quit. " "Overwrites any existing copy.",
)
flags.DEFINE_bool(
    "list",
    False,
    "List available test sets. With -t, list language pairs for "
...
flags.DEFINE_string("test_set", None, "Test set to use (see --list).", short_name="t")
flags.DEFINE_string(
    "language_pair",
    None,
...
flags.DEFINE_string(
    "input",
    None,
    "Read input from a file instead of STDIN. Each line should "
...
flags.DEFINE_string("output", None, "Output file, defaults to STDOUT.", short_name="o")
flags.DEFINE_string(
...
flags.DEFINE_string(
    "gold",
    "std",
    'Type of gold scores to compare to, use "std" to designate official '
    "gold scores.",
    short_name="g",
)
flags.DEFINE_string(
...
flags.DEFINE_float(
    "thresh",
    -1,
    "Threshold for WMT Kendall-like correlation. Defaults to 25 "
...
flags.DEFINE_bool(
    "matrix",
    False,
    "Compute correlations for a set of metrics, and perform "
...
flags.DEFINE_string("matrix_level", "sys", 'Granularity, one of "sys", "doc" or "seg"')
flags.DEFINE_string(
    "matrix_domain",
    None,
...
flags.DEFINE_bool(
    "matrix_primary", True, "Use only primary metric submissions in the matrix."
)
flags.DEFINE_float(
    "matrix_pval",
    0.05,
            ...
            for i in range(len(evs.src)):
                ...
                for level in "seg", "doc", "domain", "sys":
                    if level not in evs.levels:
                        continue
                    ind = {"seg": i, "doc": doc_id, "domain": domain_id, "sys": 0}[
                        level
                    ]
...
def PrintMatrix(new_metric_dirs):
    ...
    task_results = task.Run(parallel_file=FLAGS.matrix_parallel, eval_set_dict=evs_dict)
    fh = open(FLAGS.output, "w") if FLAGS.output else sys.stdout
    with fh:
...
def PrintComparison(res_base, res_comp, outfile):
    ...
    kendlike = corr1.AverageCorrelation(
        stats.KendallLike, "item", FLAGS.replace_nans_with_zeros, thresh=FLAGS.thresh
    )
    ...
    print(
        ...
        % (
            _Summary(pear_b, pear_w, pear_p),
            _Summary(sper_b, sper_w, sper_p),
            _Summary(kend_b, kend_w, kend_p),
            FLAGS.thresh,
            _Summary(kl_b, kl_w, kl_p),
        ),
        file=outfile,
    )
...
def main(argv):
    ...
    if FLAGS.list:
        if FLAGS.test_set is None:
            ...
            print(
                f"language pairs for {FLAGS.test_set}:",
            ...
            print(
                ...
                % (
                    ...
                    len(evs.docs),
                    len(evs.sys_names),
                    len(evs.outlier_sys_names),
                    len(evs.human_sys_names),
                    ",".join(evs.outlier_sys_names),
                    ",".join(evs.human_sys_names),
                    ",".join(evs.all_refs),
                    ",".join(evs.human_score_names),
                ),
            )
        return
    ...
    evs = data.EvalSet(
        FLAGS.test_set, FLAGS.language_pair, read_stored_metric_scores=FLAGS.scores
    )
    if FLAGS.primary_metrics:
        evs.SetPrimaryMetrics(set(FLAGS.primary_metrics))
    ...
    if FLAGS.echo is not None or FLAGS.echosys is not None:
        ...
        if FLAGS.echo is not None:
            for lines in zip(*texts, strict=False):
                print("\t".join(lines))
        else:
            for sysname, sysout in evs.sys_outputs.items():
                for lines in zip(sysout, *texts, strict=False):
