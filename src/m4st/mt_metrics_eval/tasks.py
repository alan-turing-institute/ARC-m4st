...
# Modified 2025 Katie Awty-Carroll (ARC @ The Alan Turing Institute)
#
...
def _TsvTable(metric_infos, rows, headers) -> str:
    ...
    for m, row in zip(metrics, rows, strict=False):
...
def _TextTable(metric_infos, rows, headers, rerank) -> str:
    ...
    for m, row in zip(metrics, rows, strict=False):
        row_str = [
            FormatCorrRank(c, r, rr) for (c, r), rr in zip(row, rerank, strict=False)
        ]
    ...
    for header in headers:
        ret += (
            "  ".join(_Just(h, w) for h, w in zip(header, col_widths, strict=False))
            + "\n"
        )
    if headers:
    ...
    for row in row_strs:
        ret += (
            "  ".join(_Just(r, w) for r, w in zip(row, col_widths, strict=False)) + "\n"
        )
    return ret
...
def _LatexTable(metric_infos, rows, headers, rerank) -> str:
    ...
    if headers:
        ret += "\\midrule\n"
    for metric, row in zip(metrics, rows, strict=False):
        row_str = [
            FormatCorrRank(c, r, rr) for (c, r), rr in zip(row, rerank, strict=False)
        ]
...
def MetricsTable(
    ...
    new_columns = []
    for rr, col in zip(rerank, columns, strict=False):
        if rr:
            col = sorted(
...
class TaskSet:
    ...
    def __init__(self, attr_combs: dict[str, list[Any]] | None = None, **attrs):
        ...
        for vals in itertools.product(*attr_combs.values()):
            comb = dict(zip(attr_combs.keys(), vals, strict=False))
...
class TaskSetResults:
    ...
    def AssignWeights(self, attrs: list[str], total_wt: float = 1.0) -> list[float]:
        ...
        for subset in subsets:
            ...
            for r, w in zip(subset.results, subweights, strict=False):
                weights[r.name] = w
    ...
    def AverageRanks(self, weights=None) -> dict[str, float]:
        ...
        ranks = {}
        for res, weight in zip(self.results, weights, strict=False):
    ...
    def AverageCorrMatrix(
        ...
        corrs_and_ranks = {
            m: (c, r) for (m, c), r in zip(avg_corrs.items(), ranks, strict=False)
        }
...
def WMT24(lps: list[str] | None = None, primary=True, k=0, gold=None, domain=None):
    """Generate the WMT24 task set associated weight vector.
    Modified by Katie Awty-Carroll (Alan Turing Institute) in February 2015 to
    include domain argument."""
    ...
    def Add(lp, level, corr_fcn, human, gold, domain, **kw_args):
        tasks.Append(
            Task(
                "wmt24",
                lp,
