...
import numpy as np
from mt_metrics_eval import meta_info, ratings, stats
...
class EvalSet:
    ...
    def DomainsPerDoc(self):
        ...
        domains_and_docs = zip(self.DomainsPerSeg(), self.DocsPerSeg(), strict=False)
        return [domain for domain, _ in _MapPositions(list(domains_and_docs))]

    def _ReadDataset(
        self, name, lp, read_stored_metric_scores, path, strict, read_stored_ratings
    ):
...
def GetCorrelations(
    ...
    if sample_size is not None:
        ...
        if domain is not None:
            doc_sizes = [
                s
                for d, s in zip(evs.DomainsPerDoc(), doc_sizes, strict=False)
                if d == domain
            ]
    ...
    def _Filter(scores):
        if domain is not None:
            ...
            for k, vals in scores.items():
                ...
                filt_scores[k] = [s for s, m in zip(vals, mask, strict=False) if m]
            scores = filt_scores
...
def CompareMetricsWithGlobalAccuracy(
    ...
    for base_metric in set.intersection(*(set(b) for b in base_metrics)):
        ...
        for c, metric_map in zip(corrs, base_metrics, strict=False):
...
def MakeTaskName(
    ...
    **corr_fcn_args,
    ...
    return " ".join(f"{k}={v}" for k, v in zip(locals(), vals, strict=False))
