...
import scipy.stats
from mt_metrics_eval import pce, tau_optimization
...
def filter_gold_nones(gold: ArrayLike, model: ArrayLike) -> tuple[ArrayLike, ArrayLike]:
    ...
    """
    filt = [(v1, v2) for v1, v2 in zip(gold, model, strict=False) if v1 is not None]
    if not filt:
        return [], []
    return zip(*filt, strict=False)
...
class AverageCorrelation:
    ...
    def Corr(self, gold_vect: ArrayLike, metric_vect: ArrayLike):
        ...
        with warnings.catch_warnings():
            ...
            for r1, r2 in zip(mat1, mat2, strict=False):
...
def KendallLike(
    ...
    for a, b in itertools.combinations(
        zip(gold_scores, metric_scores, strict=False), 2
    ):
...
def Agreement(gold_vect, metric_vect):
    ...
    agree, num_pairs = 0, 0
    for a, b in itertools.combinations(zip(gold_vect, metric_vect, strict=False), 2):
        if a[0] is None or b[0] is None:
            continue
...
def _ReshapeAndFilter(
    ...
    for ok, g, s1, s2 in zip(okay, gold, scores1, scores2, strict=False):
        num_elems = ok.sum()
        if num_elems:
        ...
        with warnings.catch_warnings():
            ...
            if corr_fcn is KendallWithTiesOpt:
                ...
                vals = [
                    ...
                    for (b, e), pp in zip(bounds, preprocs, strict=False)
                ]
            else:
                vals = [
...
def PairwisePermutationSigDiff(
    ...
    def _Corr(preprocs):
        vals = [
            ...
            for g, m in zip(preprocs_gold, preprocs, strict=False)
        ]
        if replace_nans_with_zeros:
            vals = np.nan_to_num(vals)
        else:
    ...
    for i in range(1, k + 1):
        ...
        hybrids1, hybrids2 = [], []
        for n, m1, m2 in zip(lens, preprocs1, preprocs2, strict=False):
...
class Sample:
    ...
    def __init__(self, total_size, size, method, bin_sizes=None, seed=None):
        ...
        if method == "uniform":
            ...
            self.sample = []
            beg = 0
            for n, s in zip(bin_sizes, size_per_bin, strict=False):
