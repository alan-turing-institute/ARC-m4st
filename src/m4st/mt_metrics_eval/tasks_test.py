...
class TaskTest(unittest.TestCase):
    ...
    def testPostInitAccuracyCorr(self):
        ...
        task = tasks.Task(test_set="wmt21.news", lang="en-de", corr_fcn="accuracy")
        ref_task = tasks.Task(
            test_set="wmt21.news",
            lang="en-de",
            corr_fcn="accuracy",
    ...
    def testRunDefault(self):
        ref_metrics = [
            ...
            "YiSi-1",
            "COMET-QE[noref]",
            "MS-COMET-QE-22[noref]",
            "MEE4",
            "MATESE-QE[noref]",
            "BERTScore",
            "HWTSC-Teacher-Sim[noref]",
...
class TaskResultsTest(unittest.TestCase):
    ...
    def testResultsString(self):
        results = (
            {"m1": (0.111111111, 1), "metric2": (0.222222222, 2)},
            np.array([[0, 0.01], [0, 0]]),
            None,
            None,
        )
    ...
    def testRange(self):
        ...
        results = tasks.Task(
            k=0, corr_fcn="KendallWithTiesOpt", corr_fcn_args={"variant": "23"}
        ).Run()
        self.assertEqual(results.range, (-1, 1))
